# Bluesky & Mastodon Daily Poster (v4.21.1)
...
- **Images** — Mentor/Strategist posts may attach an AI-generated illustration (Imagen 4; two-step — Gemini drafts the visual prompt, Imagen renders it). Curator uses an RSS link card instead.
- **Models** — a `GEMINI_MODEL_PRIORITY` chain (gemini-3.5-flash primary); an API failure advances to the next model, and the list is pruned to what the key can actually reach at startup. Thinking budget is pinned per model so it doesn't eat the output budget.
