...
- **Models** — a `GEMINI_MODEL_PRIORITY` chain (gemini-3.7-flash primary, trialing; gemini-3.5-flash the immediate fallback); an API failure advances to the next model, and the list is pruned to what the key can actually reach at startup. Thinking budget is pinned per model so it doesn't eat the output budget.
