# Bluesky & Mastodon Daily Poster (v4.17.0)
...
**Telemetry (v4.17)**: each run writes structured engagement and feed-health data to two Gist-backed JSON files. `post_metrics.json` records one row per delivered post (post_id, mode, topic, source domain, pioneer id, had_image / had_link_card, thread position) plus a metrics sub-object that gets refreshed against each platform's read API once per ~24h — Bluesky `get_posts` batched ≤25 URIs, Mastodon `status` per row. Rows older than 30 days are pruned. `feed_health.json` tracks each RSS feed's last fetch outcome with a 28-attempt rolling window (~2 weeks at 2 runs/day) so dead or drifting feeds surface in a weekly digest rather than via mystery low-volume Curator runs. Steady-state ~10–20 metrics-API calls per run; refresh and prune share the same I/O cycle as the broadcast-time recording.
