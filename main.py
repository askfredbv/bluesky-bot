...
async def _apply_curator_fallback_image(
    ...
    (compressed to fit Bluesky's blob limit). Mutates ``link_meta`` in place and
    returns the generated bytes so the caller can attach the same image to
    Mastodon, which unfurls the URL itself and does not use the Bluesky card
    thumb. Returns None — a thumbnail-free card, never a crash — if the card
    already has a thumbnail, generation returns nothing, or compression fails."""
    ...
    if not generated:
        ...
        return None
    try:
        compressed = compress_image(generated)
    except Exception as exc:
    ...
    SafeLogger.info("curator_fallback_image",
                    ...
                    topic=topic, image_bytes=len(compressed))
    return compressed
...
async def main():
    ...
    SafeLogger.info("run_started", "--- AskFred Engine v4.25.0 ---")
