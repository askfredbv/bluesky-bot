...
Rewritten in dry first-person voice matching the bot's own posts (Draft A). Both platforms now share a single 210-char string aliased via `APPROVED_BIO_MASTODON = APPROVED_BIO_BSKY` — same content, single source of truth. Auto-rolls out on the next run since `update_profile_bio` is idempotent. Original issues:
