...
1. **Phase 4b — proactive replies** [**in flight, 4/6 commits landed**]. The only audience-acquisition lever in the plan. Watchlist exists from Phase 4a (two handles: `simonwillison.net`, `xeiaso.net`). Multi-session arc — one commit per session is the working rhythm. **Shipped:** commit 1 in `0d04426` (state schema + I/O helpers, 12 tests); commit 2 in `639fa05` (`generate_proactive_reply` + SKIP contract + few-shot prompt, 17 tests); commit 3 in `f0484b6` (scan/filter/pick/stage in `src/proactive.py`, 27 tests); commit 4 in `1f3585e` (`scripts/run_proactive_scan.py` entry point + `proactive_scan.yml` workflow, 5 tests). Daily run is unchanged; kill-switch verified intact. **Workflow ships dormant — production activation requires enabling it in the Actions UI and adding a cron-job.org trigger at ~10:00 UTC.** Until then the new code is unreachable. **Next commit:** the approval path — `approve_draft` / `reject_draft` in `src/proactive.py` + `scripts/run_proactive_approve.py` + `.github/workflows/approve_pending_reply.yml`. First commit where a Phase 4b reply can actually go live on Bluesky (via explicit manual approve, not unattended). See `PLAN_engagement.md §4b` for the full map.
