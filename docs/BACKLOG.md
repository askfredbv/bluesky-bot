...
Living list of pending work and parked ideas. Bot is shipping at **v4.21.1** (2026-06-15) — generated images return to the feed (the 976 KB gate now compresses-to-fit instead of dropping; last image before the fix was 2026-06-08), the README is shortened/de-staled, and the Gemini voice-audit + AGENTS.md Codex-review sparring tools are in place. Merged since v4.21.1 without a release bump: the image model moved off Imagen 4 to **`gemini-3.1-flash-image`** (#59, live-validated on the feed 2026-06-16/17); the Phase 4b real-handle scrub completed across targeting data, few-shot examples, and test fixtures (Type A/B/C — #57/#63/#64); routine dependency bumps (#62); and the Pioneer pool broadened by 15 tech-history entries (Margaret Hamilton #60 + a 14-entry batch #65). The wiki + in-repo docs were de-staled (#66); ruff bumped to 0.16.0 (#67); and **gemini-3.7-flash was promoted to primary as a trial** — gemini-3.5-flash is now the immediate fallback, and the unreachable 1.5-flash / gemma fallbacks were dropped (#68, awaiting a feed-read to keep or revert). Since v4.20.1: the 2026-05-21 content batch (Dutch dropped, Curator rewritten to demand a take, two-register voice anchor); **gemini-3.5-flash promoted to primary** after a voice-quality trial (KEEP — faster, cheaper, a generation newer, voice intact); Pioneer cooldown 30→60; the curator link-card fix (card follows the item the model wrote about); `AGENTS.md` briefing the Codex PR reviewer on project principles; and two Codex-found state-safety hardenings in the (still-dormant) Phase 4b pipeline. Phase 4b is code-complete and dormant (all commits landed; workflows ship disabled; production activation requires manual user steps and is deliberately deferred). Nothing below is urgent.
...
- **`pyproject.toml` migration trigger.** Dev-tool count is at 3 (ruff, pytest, pytest-asyncio) sitting in `requirements.in` alongside runtime deps. Architecturally weird (the daily workflow installs ruff into every production run) but harmless at this scale. **Migrate when a 4th non-pytest-flavoured dev tool lands** — mypy, pre-commit, pyright, types-*, pytest-cov, etc. At that point `requirements.in` starts looking noisy and the runtime/dev split pays for itself. Target shape: single `pyproject.toml` with `[project.dependencies]` for runtime + `[project.optional-dependencies] dev = [...]`; CI runs `pip install '.[dev]'`, daily workflow runs `pip install .`. Dependabot already supports the format. ~30–60 min mechanical migration; no behaviour change. Surfaced 2026-05-21 during the PR #47 cleanup that uncovered ruff being silently dropped by Dependabot's lockfile regen because it wasn't in `requirements.in`.
