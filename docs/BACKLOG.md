...
- 2026-09-03: **News coverage: stop burying obvious launches + primary-source feeds + feed-health alert.** Prompted by missing an obvious Google model launch. Root cause was the magnitude-blind **−12 topic-diversity penalty** in `calculate_relevance_score` — it demoted a flagship launch just because we'd posted on its topic recently. Fix: a **landmark** gate (a `MOMENTUM_PRODUCTS` flagship that is either framed as a launch — new `LAUNCH_SIGNAL_KEYWORDS` — or covered by ≥`LANDMARK_CONSENSUS_MIN_PUBLISHERS` independent publishers) **waives the diversity penalty and adds `LANDMARK_LAUNCH_BONUS` (+6)**. Anchored to a named flagship so routine topic-repeats never qualify. **Feeds:** removed the dead `www.anthropic.com/news.rss` (404 since the claude.com rebrand — no usable replacement exists, so it had been silently failing every run); added verified primary-source blogs — `blog.google/technology/ai/rss/`, `blog.google/products/gemini/rss/`, `mistral.ai/rss.xml`, `developers.openai.com/rss.xml`, `blogs.nvidia.com/feed/` — with matching `SOURCE_TIERS`, so flagship launches enter the pool first-hand (fresher, higher-tier) instead of via later secondary coverage. **Feed-health alert:** `check_feed_health_alerts` now WARNs on any feed that has failed — or 200s-but-yields-nothing (the 404-as-HTML shape) — across the recent window, so the next silent feed death surfaces on its own rather than as missing coverage weeks later. **Image:** `IMAGE_GENERATION_PROBABILITY` 1.0 → 0.85 for cadence variety (~1 in 7 posts text-only). 487 tests pass (+13), ruff + mypy clean, coverage 86.7%.
