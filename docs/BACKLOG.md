...
- **`pyproject.toml` migration trigger — FIRED 2026-08-29, migration now due.** The threshold was "migrate when a 4th non-pytest-flavoured dev tool lands"; the 2026-08-29 audit added both `pytest-cov` (CI install line) and `mypy` (pinned `>=2.3,<3` in `requirements.in`), so `requirements.in` now mixes runtime deps with ruff + pytest + pytest-asyncio + mypy and the daily production workflow still installs the lot. **This is now the trigger-met action, not a someday.** Target shape: single `pyproject.toml` with `[project.dependencies]` for runtime + `[project.optional-dependencies] dev = [...]` (ruff, pytest, pytest-asyncio, pytest-cov, mypy); CI runs `pip install '.[dev]'`, daily workflow runs `pip install .`. Dependabot already supports the format. ~30–60 min mechanical migration; no behaviour change. Surfaced 2026-05-21 during the PR #47 cleanup that uncovered ruff being silently dropped by Dependabot's lockfile regen because it wasn't in `requirements.in`.
