...
1. **Phase 4b — proactive replies** [**in flight, 3/6 commits landed**]. The only audience-acquisition lever in the plan. Watchlist exists from Phase 4a (two handles: `simonwillison.net`, `xeiaso.net`). Multi-session arc — one commit per session is the working rhythm. **Shipped:** commit 1 in `0d04426` (state schema + I/O helpers, 12 tests); commit 2 in `639fa05` (`generate_proactive_reply` + SKIP contract + few-shot prompt, 17 tests); commit 3 in `f0484b6` (scan/filter/pick/stage in `src/proactive.py`, 27 tests). Daily run is unchanged; kill-switch from `a2cd1f1` verified intact (no `main.py` imports of `src/proactive.py`). **Next commit:** workflow YAML + `scripts/run_proactive_scan.py` entry point wiring `scan_watchlist → generate_proactive_reply → stage_draft_reply → save_pending_replies` into a daily ~10:00 UTC scheduled run. First commit where drafts start staging in production. See `PLAN_engagement.md §4b` for the full map.
